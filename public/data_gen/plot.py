...
def normalize(im, target_min=0.0, target_max=1.0):
    max_vals = np.amax(im, keepdims=True)
    min_vals = np.amin(im, keepdims=True)
    zero_one = (im - min_vals) / (max_vals - min_vals)
    return (zero_one * (target_max - target_min)) + target_min

def image_plot(shap_values, x, labels=None, show=True, width=20, aspect=0.2, hspace=0.2, take_abs=False, as_alpha=False, reverse=False, max_percentile=99.9, labelpad=None, class_names=None, bar=True, orig_image_overlay=True, orig_image_left=True, as_array=True):
    ...
    if type(shap_values) != list:
        if as_array:
            shap_values = [np.squeeze(arr, axis=1) for arr in np.split(shap_values, shap_values.shape[1], axis=1)]
        else:
            multi_output = False
    ...
    if labels is not None:
        ...
        if multi_output:
            ...
            assert labels.shape[1] == 1, "Labels must be a vector for single output shap_values."
    ...
    for row in range(x.shape[0]):
        ...
        for i in range(len(shap_values)):
            ...
            if take_abs:
                ...
                if len(x_curr.shape) == 2:
                    x_curr = np.expand_dims(x_curr, axis=2)
                    x_curr = np.tile(x_curr, (1, 1, 3))
